...
Guidance for agent work in this repository ΓÇö a fork of `mcp-servers-for-revit` that extends the upstream Revit MCP plugin with an **off-axis line inaccuracy detection and remediation toolkit**.
...
| `commandset/` | C# `RevitMCPCommandSet` ΓÇö core command set (compiles to `RevitMCPCommandSet.dll`). |
| `offaxis-commandset/` | C# `OffAxisCommandSet` ΓÇö the off-axis detection/fix command set (compiles to `OffAxisCommandSet.dll`), plus standalone ZIP distribution. |
...
The off-axis detector/fixer logic is **promoted into compiled C# commands** (see ┬ºOff-Axis Toolkit). The original Roslyn `.csx` prototype scripts are **frozen** in a separate archive repo (see ┬ºFrozen Script Archive) ΓÇö do not treat them as live source.
...
- **`offaxis-commandset/Commands/OffAxis/*.cs`** ΓÇö command declarations.
- **`offaxis-commandset/Services/OffAxis/*.cs`** ΓÇö `ExternalEventCommandBase` + `IExternalEventHandler`/`IWaitableExternalEventHandler` handlers implementing the logic.
- **`offaxis-commandset/Utils/OffAxis/`** ΓÇö `OffAxisGeometryUtils.cs` and `SilentFailuresPreprocessor.cs`.
- **`offaxis-commandset/command.json`** ΓÇö command set declaration (name `OffAxisCommandSet`, developer Scott Mitchell Studio).
...
1. **Full Plugin Installer** (`installer/output/mcp-servers-for-revit-setup.exe`) ΓÇö built by `installer/build-installer.ps1`. Installs the add-in, both command sets, and configures AI clients (Claude Desktop, Cursor, opencode). `AppVersion` is **derived from the plugin** (`plugin/Properties/AssemblyInfo.cs` `AssemblyFileVersion`), not hardcoded.
2. **Standalone CommandSet ZIP** (`offaxis-commandset/dist/OffAxisCommandSet.zip`) ΓÇö built by `offaxis-commandset/build-package.ps1`. Contains `OffAxisCommandSet`, `install.ps1`, `uninstall.ps1`, `README.md`. Merges 10 commands into an existing plugin's `commandRegistry.json`, or can be dropped in via Revit MCP Settings -> "Open CommandSet Folder" -> Refresh -> Save.

The installer exposes a **Command Sets selection step** (pre-selects all) plus an AI-client config step.

### Deployment Gotcha ΓÇö Installer overwrites AI-client configs

Running the installer re-runs `ConfigureClaudeDesktop` / `ConfigureCursor` / `ConfigureOpencode` in `[Code]`. Critically, `ConfigureOpencode` **overwrites `~/.config/opencode/opencode.json` back to `npx -y mcp-server-for-revit`** ΓÇö the **broken npm package** that crashes on startup with `Cannot find module 'ajv'` (MCP error 32000). The published `mcp-server-for-revit@1.0.0` was never re-published with the `ajv: ^8.17.1` fix (user is not an npm owner; fix is local-only). So any reinstall breaks the opencode MCP connection until the config is pointed back at the local build:

```jsonc
// ~/.config/opencode/opencode.json ΓÇö point at the LOCAL build
...
Verify the local server boots with a manual MCP handshake (`node server/build/index.js` from `server/`; respond to `initialize`; 33 tools register). Note: `better-sqlite3` has **no prebuilt binary for Node 26** and this machine lacks node-gyp/Python/VS, so the 3 sqlite-backed tools (`query_stored_data`, `store_project_data`, `store_room_data`) fail to register ΓÇö the other 33 tools and the connection work fine.

### Future Work ΓÇö Dynamic tool advertisement (planned, not yet implemented)

The Node server's tool list is **static** (`server/src/tools/register.ts` imports every tool file at startup) while the plugin only dispatches commands present in `Commands\commandRegistry.json` (`RevitCommandRegistry.TryGetCommand` at request time; unknown methods return `Method not found`). Result: the server advertises ~34 tools regardless of what Revit actually registered ΓÇö users see "listed but unreachable" tools. Desired end state: on socket connect (or handshake), the plugin sends its registered command list, and the Node server filters/folds its tool list accordingly (e.g., skip tool registration or return a clear "command not enabled ΓÇö enable in Settings" result). Requires touching `server/src/utils/SocketClient.ts`, `server/src/index.ts` (dynamic registration after connect), and a plugin-side advert message in `plugin/Core/SocketService.cs`. Coordinate with the existing static-registration code path so stdio client UX (tool list in client UI) stays stable when the plugin is briefly offline.
...
- **Large-Fix Flags**: `FLAG_MOVEMENT_IN = 0.5` in; `FLAG_DEVIATION_DEG = 0.1` deg; predicted swing `= 2┬╖L┬╖sin(╬ö╬╕/2)`. Any element exceeding either threshold ΓåÆ `LargeFix = true`.
...
- **In-place strategies**: (1) length-preserving midpoint rotation vs an **18-direction** world-axis candidate set (3 axes ┬▒ and 6 diagonals ┬▒) ΓÇö all 18 must be present else a true 135┬░ diagonal reports ~45┬░ and never snaps; (2) fallback corner-vertex ray-ray closure.
...
- **Sketch access differences**: Floors/Ceilings ΓåÆ `((Floor)el).SketchId` ΓåÆ `Sketch.Profile` (`CurveArrArray`). FootPrint Roofs ΓåÆ no `.SketchId`; use `roof.GetProfiles()` (`ModelCurveArrArray`).
...
- **`IsJoined`/`IsJoinedWith` do not exist** in this build ΓÇö use `JoinGeometryUtils.GetJoinedElements(doc, el)`.

### Pass B ΓÇö 1/4" Lattice Spacing Regularizer

- **Lattice math**: for a segment parallel (┬▒0.1┬░) to a grid with signed perpendicular offset `p`, phase `╧å = p ΓêÆ G┬╖floor(p/G)` (`G = 0.25 in`); nearest lattice line `s_n = ╧å + G┬╖round((pΓêÆ╧å)/G)`; deviation `= (p ΓêÆ s_n)┬╖12` in. `DeltaIn = 0` Γçö on-lattice.
...
- **Fixer = live recompute, never CSV deltas**: after batch moves, Revit joint cleanup silently steals part of the move. Recompute current `p ΓåÆ ╧å ΓåÆ s_n ΓåÆ` shift on the fly, snap-commit, re-read, self-correct up to 3 passes.
- **CRITICAL**: never call `SetProcessingResult(ProceedWithCommit)` when there are no errors ΓÇö it triggers an infinite failure-processing loop / UI lock / MCP timeout.
- **Batching**: keep batches to **10ΓÇô12 element IDs per MCP call** to stay under the 120s window in large models.
...
- When dispatching to the compiled tools, pass `elementIds`/`hostIds`/`lineIds` as JSON arrays or CSV strings. For spacing passes use 10ΓÇô12 IDs per call; all warnings/errors are handled headlessly by the `FailuresProcessing` delegate.
...
- Repo targets Revit 2020ΓÇô2026 (R20ΓÇôR26).
