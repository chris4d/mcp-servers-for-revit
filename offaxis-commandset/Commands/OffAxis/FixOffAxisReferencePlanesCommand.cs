...
{
    /// <summary>
    /// Snaps slightly off-axis vertical reference planes to nearest 0/45/90 orientation.
    /// </summary>
    public class FixOffAxisReferencePlanesCommand : ExternalEventCommandBase
    {
        private FixOffAxisReferencePlanesEventHandler _handler => (FixOffAxisReferencePlanesEventHandler)Handler;

        public override string CommandName => "fix_off_axis_reference_planes";

        public FixOffAxisReferencePlanesCommand(UIApplication uiApp)
            : base(new FixOffAxisReferencePlanesEventHandler(), uiApp)
        {
        }
        ...
        public override object Execute(JObject parameters, string requestId)
        {
            try
            {
                ...
                {
                    throw new TimeoutException("fix_off_axis_reference_planes operation timed out");
                }
            }
            catch (Exception ex)
            {
                throw new Exception($"Failed to fix off-axis reference planes: {ex.Message}", ex);
            }
        }
    }
}
